# MIS 443 - Business Data Management

The repository for **MIS 443 - Business Data Management** practical assignments 

---

## 📂 Course Practical Assignments

Below is the list of all practical assignments organized into separate directories:

### 1. Individual Assignments
* **[Assignment 1: Database Setup & Load](./2232300060_Than%20Que%20Ngoc_MIS443_Assignment%201.pdf)** - Environment setup, database loading, and basic query exploration to identify characteristics of tables.
  * [👉 View SQL Script](./2232300060_Than%20Que%20Ngoc_MIS443_Assignment%201.sql)


### 2. Group Activities
* **[Activity 1: Banking Database](./MIS%20443_Activity%201_Group%20D2NB.pdf)** - Relational design and SQL practice queries on a commercial banking database.
  

### 3. Additional Self-Practice
* **[Banking Analysis](./banking%20(individual).sql)** - Relational design and SQL practice queries on a commercial banking database **(Individual)**
  * [👉 View PDF Document](./banking%20(individual).pdf)
 
* **[PostgreSQL to Python Connection](./postgresql-python-connection.ipynb)** - Database Connectivity




 

## 🛠️ Technologies & Tools Used
* **Database Management System:** PostgreSQL (v18)
* **Database Administration Tool:** pgAdmin 4 
